# 2026-05-09: Fix spacing paragraph chứa drawing GVCN (5 template)
# Old: <w:spacing w:before="120"/>...<w:jc w:val="center"/></w:pPr><w:r><w:rPr><w:noProof/></w:rPr><w:drawing>
...
$backup = Join-Path $root "_backup-pre-spacing-fix-2026-05-09"
if (-not (Test-Path $backup)) { New-Item -ItemType Directory -Path $backup | Out-Null }

$oldPattern = '<w:spacing w:before="120"/><w:ind w:left="54" w:right="325"/><w:jc w:val="center"/></w:pPr><w:r><w:rPr><w:noProof/></w:rPr><w:drawing>'
$newPattern = '<w:spacing w:before="240" w:after="240"/><w:ind w:left="54" w:right="325"/><w:jc w:val="center"/></w:pPr><w:r><w:rPr><w:noProof/></w:rPr><w:drawing>'
...
1..5 | ForEach-Object {
    ...
    try {
        ...
        if ($xml -notmatch [regex]::Escape($oldPattern)) {
