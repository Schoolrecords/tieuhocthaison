...
- **Xã hành chính**: Văn Hiến · **Tỉnh**: Nghệ An
- **Địa danh ký văn bản**: `"Thái Sơn,"` (KHÔNG dùng xã hành chính `"Văn Hiến,"` sau sáp nhập). Áp dụng cho mọi văn bản: học bạ, báo cáo chất lượng, công văn, biên bản. Xem `app.js:renderCatalogOverview` (chữ ký bìa tổng quan HSS) + `qlcl-app.js:_renderHocBa1HS` (chữ ký học bạ).
